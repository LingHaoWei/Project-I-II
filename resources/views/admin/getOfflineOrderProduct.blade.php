...
<div class="content" id="pwrapper1">
  ...
  <div class="form addProForm row">
        ...
        <form method="POST" , action="{{route('addOfflineOrder')}}" enctype="multipart/form-data" id="dynamic_form">
        @csrf
        <div class="form-group addProRow1">
            

            <label class="" for="Document No">Order No</label>
            <div class="">
                <input type="text" class="form-control" id="OrderNo" name="OrderNo" value="{{$docno}}" readonly>
            </div>
        ...
        <label class="" for="Document No">Invoice No</label>
            <div class="">
                <input type="text" class="form-control" id="InvoiceNo" name="InvoiceNo" value="{{$invno}}">
            </div>
        </div>
        ...
        <tbody>
                    ...
                    <select name="chooseProduct" id="chooseProduct" class="form-control chooseProduct" onchange="this.options[this.selectedIndex];">
                        ...
                        <a href=""><option value="{{$product->productID}}" data-name="{{$product->name}}" data-price="{{$product->price}}">{{$product->name}} ({{$product->productID}})</option></a>
                        @endforeach
                    </select>
                    </td>
                    <td></td>
        ...
        <label class="" for="PurchaseOrder Notes">Notes</label>
            ...
            <Button type="button" class="backBtn">
                <a href="{{ route('viewOfflineOrder') }}" class="" title="Back" data-toggle="tooltip">Back</a>
            </Button>
